...
export GXCONTAINER="/software/projects/pawsey0272/ykzhang/container/gx_container_25jul.img"  # Absolute path to the GLEAM-X singularity container, including the file name, e.g. "${GXSCRATCH}/gleamx.img"
...
export GXABSMEMORY=100           # Absolute memory a machine should be considered to have in GB, e.g. 60. This value is submitted to slurm via "--mem=${GXABSMEMORY}"
...
export GXMEMORY=100              # Typical memory a program should use in GB, e.g. 50. This is used for tasks like 'calibrate' and 'wsclean' to attempt to limit
...
then
    ...
    if [[ ! -d ${GXMWALOOKUP} ]]
    then
	# this cloud is no longer able to be used
