...
# Basic configuration
cluster="petrichor"            # System-wide name of cluster, e.g. "garrawarla". This does not have to have a one-to-one correspondance to the
                                # actual system name.
...
export GXBASE="/scratch1/$GXUSER/GLEAM-X-pipeline" # Path to base of GLEAM-X Pipeline where the repository was 'git clone' into, e.g. "/astro/mwasci/tgalvin" 
export GXSCRATCH="/scratch1/$GXUSER"     # Path to scratch space used for processing on the HPC environment, e.g. /scratch
...
export GXCONTAINER='/scratch1/$GXUSER/gleamx_testing_small.img'   # Absolute path to the GLEAM-X singularity container, including the file name, e.g. "${GXSCRATCH}/gleamx.img"
...
export GXSTANDARDQ="defq"      # Slurm queue to submit tasks to, e.g. "workq". Available queues can be inspected using 'sinfo' on a system where the slurm schedular is available
...
export GXNCPUS=8               # Number of CPUs of each machine, e.g. 48. For tasks that have a 'core' like option this value is passed. 
export GXNLCPUS=8              # Number of logical CPUs on each machine, e.g. 48. This option may be phased out. 
export GXNPCPUS=8              # Number of physical CPUs on each machine, e.g. 24. This is meant to count only the physical cores available. 
...
export GXTRACK='no'                       # Directive to inform task tracking for meta-database. 'track' will track task progression. Anything else will disable tracking. 
...
export GXCOPYQ='io'      # A required parameter directing the job to a particular queue on $GXCOPYM. Set as just the queue name, e.g. 'copyq'
export GXCOPYM='petrichor'       # A required parameter directing the job to be submitted to a particular machine. Set as just the machine name, e.g. 'zeus'
