...
usage()
{
echo "obs_zip_ms.sh [-d dep] [-p project] [-a account] [-u user] [-t] obsnum
...
output="${GXLOG}/zip_ms_${obsnum}.o%A"
error="${GXLOG}/zip_ms_${obsnum}.e%A"
...
for taskid in $(seq ${numfiles})
    ...
    then
    # record submission
    ${GXCONTAINER} track_task.py queue --jobid="${jobid}" --taskid="${taskid}" --task='zip' --submission_time="$(date +%s)" --batch_file="${script}" \
