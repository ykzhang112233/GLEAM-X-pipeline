...
usage()
{
echo "obs_zip_folder.sh [-d dep] [-p project] [-a account] [-u user] [-t] obsnum
...
script="${GXSCRIPT}/zip_folder_${obsnum}.sh"
cat "${GXBASE}/templates/zip_folder.tmpl" | sed -e "s:OBSNUM:${obsnum}:g" \
                                 -e "s:BASEDIR:${base}:g" \
                                 -e "s:ZIPMODE:${mode}:g" \
                                 -e "s:PIPEUSER:${pipeuser}:g"  > "${script}"

output="${GXLOG}/zip_folder_${obsnum}.o%A"
error="${GXLOG}/zip_folder_${obsnum}.e%A"
...
for taskid in $(seq ${numfiles})
    ...
    then
    # record submission
    ${GXCONTAINER} track_task.py queue --jobid="${jobid}" --taskid="${taskid}" --task='zip_folder' --submission_time="$(date +%s)" --batch_file="${script}" \
